...
namespace PinballDoubleMaxMP
{
	public partial class Form1 : Form
	{
		...
		private void runSimulation_Click(object sender, EventArgs e)
		{
			...
			const int first = 1;
			for (int loopy = first; loopy < first + 0; loopy++)
			{
				...
				DateTime currentTime = new DateTime(2025, 10, 1, 11, 0, 0); // Start time of the simulation
				Queue queue = new Queue();
				Queue shadowQueue = new Queue();
				Random r1 = new Random(loopy);
				...
				while (!qualifyingOver)
				{
					...
					if (currentTime > new DateTime(2025, 10, 1, 17, 25, 0))
					{
					...
					for (int k = 0; k < shadowMatchQueue.Count;)
					{
						...
						{
							if (player.isActive || ((currentTime - player.breakStart).TotalMinutes < (double)minBreak.Value))
							{
					...
					{
						if ((currentTime - player.breakStart).TotalMinutes >= (double)minBreak.Value)
						{
					...
					for (int k = 0; k < playable1.players.Count;)
					{
						Player player = playable1.players[k]; // Get the first player in the playable queue

						// If a player completed their rounds, their qualifying is over.
						if (player.roundCount >= roundCount.Value)
						{
							playable1.RemovePlayer(player); // Remove from playable queue
							queue.RemovePlayer(player); // Remove from main queue
							continue;
						}

						// If a player is in their last round, they go to the shadow match queue.  See below.
						else if (player.roundCount == roundCount.Value - 1)
						{
							shadowQueue.AddPlayer(player);
							playable1.RemovePlayer(player); // Remove from playable queue
							queue.RemovePlayer(player); // Remove from main queue
							continue;
						}
					...
					while (playable1.players.Count > 0)
					{
						...
						// Clear opponents if they have played against at least 80% of the players or if there are less than 4 players that the player hasn't played against.
						if (player.opponents.Count >= players.Count * 4 / 5 || player.opponents.Count >= players.Count - 4)
						{
						...
						else
						{
							...
							for (int i = 0; i < 3; i++)
							{
								int j = r1.Next(opponentsNotPlayed.Count);
								Player opponent = opponentsNotPlayed[j];
								opponentsNotPlayed.RemoveAt(j);
								playable1.RemovePlayer(opponent);
								queue.RemovePlayer(opponent);
								matchPlayers.Add(opponent);
								opponent.isActive = true; // Mark the opponent as active
							}
					...
					while (shadowQueue.players.Count > 0)
					{
						...
						shadowQueue.RemovePlayer(player); // Remove the player from the shadow queue
					}
				...
				{
					textWriter.WriteLine("Loop " + loopy + ":  No player round miscounts.  Min shadow round:  " + minShadowRound);
				}
				foreach (Player player in players.Where(p => p.roundCount != roundCount.Value))
				{
					textWriter.WriteLine("WARNING on loop " + loopy + ":  Player " + player.id + " has completed " + player.roundCount + " rounds, but should have completed " + roundCount.Value + " rounds.");
				}
			}
			textWriter.Close();
